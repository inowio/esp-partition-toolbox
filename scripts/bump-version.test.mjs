...
describe("rewriteVersionInCargoLock", () => {
  it("updates only the named package", () => {
    const input = [
      ...
      "",
      "[[package]]",
      'name = "esp-partition-toolbox"',
      'version = "0.1.0"',
      "dependencies = []",
      "",
    ].join("\n");
    const out = rewriteVersionInCargoLock(input, "0.2.0", "esp-partition-toolbox");
    expect(out).toContain('name = "esp-partition-toolbox"\nversion = "0.2.0"');
    expect(out).toContain('name = "other-crate"\nversion = "1.0.0"');
  });

  it("throws when the package is missing", () => {
    expect(() =>
      rewriteVersionInCargoLock("", "0.2.0", "esp-partition-toolbox"),
    ).toThrow();
  });
});
...
describe("git-state checks (temporary repo)", () => {
  ...
  beforeEach(() => {
    ...
    execSync('git config user.email "info@inowio.in"', { cwd: tmpDir });
