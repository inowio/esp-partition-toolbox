# Releasing ESP Partition Toolbox
...
> **The updater keypair already exists.** It was generated when this repo's
> release pipeline was set up — the public key is committed in
> [`src-tauri/tauri.conf.json`](../src-tauri/tauri.conf.json)
> (`plugins.updater.pubkey`) and the private key file lives outside the repo
> at `~/.tauri/esp-partition-toolbox-updater.key`. **Steps 1 and 2 below are
> already done.** Skip to [Set the CI secrets](#3-set-the-ci-secrets) — that
> is the only one-time task still outstanding. Steps 1–2 are kept here for
> [key rotation](#updater-key-rotation).
...
```bash
npx tauri signer generate -w ~/.tauri/esp-partition-toolbox-updater.key
```
...
Replace the value of `plugins.updater.pubkey` in
[`src-tauri/tauri.conf.json`](../src-tauri/tauri.conf.json) with the public
key the command printed. Commit and push the change on `main` **before**
...
```bash
gh secret set TAURI_SIGNING_PRIVATE_KEY < ~/.tauri/esp-partition-toolbox-updater.key
...
If `main` is protected, every release goes through a one-commit release
branch and a PR. The tag is created **after** the PR merges, on the
...
- Updates `package.json`, `src-tauri/tauri.conf.json`, `src-tauri/Cargo.toml`,
  and the `esp-partition-toolbox` entry in `src-tauri/Cargo.lock`.
...
and **squash-merge** it. After the merge:
...
[releases page](https://github.com/inowio/esp-partition-toolbox/releases)
with installers, `.sig` files, and `latest.json` attached.
...
```bash
echo test > t.txt
npx tauri signer sign -f ~/.tauri/esp-partition-toolbox-updater.key -p "<YOUR-PASSWORD>" t.txt
```
