...
// The component reads/writes the clipboard through the Tauri plugin (the Rust
// backend), so mock that module — tests don't run inside the Tauri runtime.
...
describe("EditableContextMenu", () => {
  ...
  it("opens on a number input (the Size field)", () => {
    render(
      <>
        <input data-testid="num" type="number" defaultValue="128" />
        <EditableContextMenu />
      </>,
    );
    fireEvent.contextMenu(screen.getByTestId("num"), { clientX: 10, clientY: 10 });
    expect(screen.getByRole("menu")).toBeInTheDocument();
  });
  ...
  it("Copy on a number input copies the whole value", async () => {
    render(
      <>
        <input data-testid="num" type="number" defaultValue="128" />
        <EditableContextMenu />
    ...
    fireEvent.click(screen.getByRole("menuitem", { name: /Copy/ }));
    await waitFor(() => {
      expect(writeText).toHaveBeenCalledWith("128");
    });
  });
  ...
  it("survives a clipboard write failure without mutating the input", async () => {
    ...
    fireEvent.click(screen.getByRole("menuitem", { name: /Cut/ }));
    await waitFor(() => expect(screen.queryByRole("menu")).toBeNull());
    // Cut must not delete the text when the clipboard write failed.
