...
// Input types that carry no editable text — the custom menu ignores them.
...
// type="number" inputs return null (or throw) for selectionStart/End and
// reject setSelectionRange. For those the whole value is treated as the
// working range, so Copy/Cut/Paste still operate on the field as a whole.
function getRange(target: EditableTarget): { start: number; end: number; ranged: boolean } {
  ...
  try {
    ...
    // Some input types throw instead of returning null — handled below.
  }
  if (start == null || end == null) {
    return { start: 0, end: target.value.length, ranged: false };
  }
  return { start, end, ranged: true };
}

function applyNativeValue(target: EditableTarget, newValue: string, caret: number): void {
  ...
  try {
    ...
    // type="number" and similar inputs reject setSelectionRange — ignore.
  }
}

// Clipboard access goes through the Tauri plugin (the Rust backend) rather
// than navigator.clipboard, so the WebView never shows a permission prompt.
async function readClipboard(): Promise<string | null> {
  try {
    return await readText();
  } catch {
    // Read can fail outside the Tauri runtime — caller treats null as empty.
  }
  return null;
}
...
async function writeClipboard(text: string): Promise<boolean> {
  try {
    ...
    // Fall through to false so Cut leaves the field untouched on failure.
  }
  return false;
}

/**
 * A desktop-style right-click menu for text inputs and textareas. The browser's
 * native context menu is suppressed app-wide (see App.tsx); this restores the
 * useful clipboard actions in a styled menu that matches the app's theme.
 */
export default function EditableContextMenu() {
  ...
  const { start, end, ranged } = getRange(target);
  const hasContent = target.value.length > 0;
  // Without a range API the whole value acts as the selection.
  const hasSelection = ranged ? end > start : hasContent;
  ...
  async function doCut() {
    if (target && hasSelection) {
      ...
      if (ok) {
        applyNativeValue(target, target.value.slice(0, start) + target.value.slice(end), start);
      }
    }
    close();
  }
  ...
  function doSelectAll() {
    if (target) {
      ...
      try {
        ...
        // Number inputs reject setSelectionRange — select() still works.
        target.select();
      }
    }
    close();
  ...
  // Clamp to the viewport so the menu never overflows the window edges.
  const MENU_W = 176;
